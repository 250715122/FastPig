...
| 快捷键 | 功能 | 说明 |
|--------|------|------|
| `Alt + S` | 上传数据库到云端 | 将本地数据库上传到坚果云 |
| `Alt + U` | 从云端下载数据库 | 从坚果云下载并覆盖本地数据库 |
...
| `Ctrl + F` | 页内搜索 | 打开/关闭顶部搜索栏，支持高亮与上下定位 |
| `Alt + P` | 切换预览模式 | 开启/关闭 Markdown 预览 |
| `Alt + D` | 软删除当前笔记 | 删除后60秒内可撤销 |
| `Alt + Z` | 撤销删除 | 恢复最近一次删除的笔记 |
| `Ctrl + E` | 悬浮工具条 | 选中文字后弹出：加粗/斜体/删除线/H1~H3/代码/链接/图片/颜色 |
| `Ctrl + B` | 加粗 | 将选中文本包裹为 **…** |
| `Ctrl + Shift + R` | 标红 | 将选中文本包裹为 <span style="color:#e53935">…</span> |
| `Ctrl + R` | 批量替换 | 打开查找替换面板，支持正则表达式和大小写敏感 |
| `Ctrl + 1..5` | 标题1~5 | 将所选行设置为 # ~ ##### |
| `Tab` / `Shift+Tab` | 列表缩进/反缩进 | 对选中列表行每次加/减两个空格，支持多行 |
| `Enter` | 列表续项 | 在列表行自动续项；若当前行只有标记则退出该层 |
...
| `/` | 打开斜杠菜单 | 插入代码块/表格/公式/列表/引用/图片/绘图模板等 |
| `Alt + Shift + Down` | 向下添加光标 | 在下一行相同列位置添加光标（多光标编辑） |
| `Alt + Shift + Up` | 向上添加光标 | 在上一行相同列位置添加光标（多光标编辑） |
| `Alt + 鼠标拖动` | 矩形选区 | 按住 Alt 键拖动鼠标创建矩形选区，自动进入多光标模式 |
| `Ctrl + ,` | 打开设置 | 配置云同步、编辑器、界面等选项 |
| `Esc` | 关闭建议列表/退出多光标 | 关闭自动补全弹窗或退出多光标模式 |
...
5. **粘贴富文本**
   - 粘贴带格式内容时自动弹出选项对话框
   - **键盘导航**：使用 ↑↓ 方向键选择选项
   - **快速确认**：按 Enter 键确认当前选项
   - **快速取消**：按 Esc 键取消粘贴
   - 可选择：转为 Markdown / 纯文本 / 保留 HTML / 取消
   - 支持鼠标双击确认
